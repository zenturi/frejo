...
class Graphics {
	...
	var vg:VG;
	var removedCommands:Array<DrawCommand>;
	...
	public function clear() {
		removedCommands = removedCommands.concat(commands);
		commands = [];
	}
	...
	public function undo() {
		if (commands.length > 0) {
			if (removedCommands.length > 0) {
				removedCommands.push(commands.pop());
			} else {
				removedCommands = removedCommands.concat(commands);
			}
		} else {
			commands = commands.concat(removedCommands);
		}
	}
	...
	public function redo() {
		if (removedCommands.length > 0) {
			if (commands.length > 0) {
				commands.push(removedCommands.pop());
			} else {
				commands = commands.concat(removedCommands);
			}
		}
